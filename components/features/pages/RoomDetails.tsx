...
const RoomDetails = () => {
  const { id } = useParams();
  const [room, setRoom] = useState<(typeof roomsDummyData)[number] | null>(
    null,
  );
  ...
  useEffect(() => {
    if (id) {
      const foundRoom = roomsDummyData.find(
        (room: (typeof roomsDummyData)[number]) => room._id === id,
      );
      if (foundRoom) {
        setRoom(foundRoom);
          ...
          {room?.images.length > 1 &&
            room.images.map(
              (image: string | StaticImageData, index: number) => {
                ...
                cursor-pointer ${mainImage === source && "outline-3 outline-orange-500"}`}
                  />
                );
              },
            )}
          ...
          <div className="flex flex-wrap items-center mt-3 mb-6 gap-4">
            {room.amenities.map((item: string, index: number) => {
              ...
              return (
                ...
                >
                  <Image src={icon} alt={item} className="w-5 h-5" />
